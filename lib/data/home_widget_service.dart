...
class HomeWidgetService {
  ...
  Future<void> _syncNextAndToday(List<ClaseHorario> horario) async {
    final now = DateTime.now();
    final today = now.weekday;

    // Próxima clase (hoy o próximos días).
    ...
    for (var off = 0; off < 8; off++) {
      ...
      final list = horario.where((c) => c.idDia == dia).toList()
    ...
    // Cursos de hoy (agrupados).
    final hoy = horario.where((c) => c.idDia == today).toList();
    final grupos = ClaseAgrupada.agrupar(hoy);
    final buf = StringBuffer();
    for (final g in grupos.take(6)) {
      buf.writeln('${g.horaInicio}  ${g.asignatura}');
    }
    await _save(
      'today_list',
  ...
  Future<void> _syncPayment(List<Cuota> cuotas) async {
    final sorted = [...cuotas]..sort((a, b) {
        final da = a.vencimientoDate, db = b.vencimientoDate;
    ...
    if (sorted.isEmpty) {
      ...
      await _save('pay_desc', c.descripcion);
      await _save('pay_amount',
          '${c.tipoMoneda} ${c.subtotal.toStringAsFixed(2)}');
      await _save(
        'pay_due',
        d == null
            ? c.fechaVencimiento
            : d < 0
                ...
                : d == 0
                    ? 'Vence hoy'
                    : 'Vence en $d d (${c.fechaVencimiento})',
      );
    }
