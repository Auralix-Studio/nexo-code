...
class ErrorHandler {
  ...
  /// Executes [remote] operation, with automatic fallback to [cached] on failures.
  /// Handles SessionExpiredException and logs errors.
  Future<T> withFallback<T>({
    ...
    try {
      if (!connectivity.hasInternet) {
        debugPrint('ErrorHandler: No internet for $operationName, falling back to cache');
        final cachedResult = await cached();
        if (cachedResult != null) return cachedResult;
        throw const NetworkException('Sin conexión a internet y no hay datos locales de respaldo.');
      }
      ...
      debugPrint('ErrorHandler: Server exception in $operationName: $e. Trying cache.');
      final cachedResult = await cached();
      if (cachedResult != null) return cachedResult;
      rethrow;
    } on DataParsingException catch (e) {
      debugPrint('ErrorHandler: Data parsing exception in $operationName: $e. Trying cache.');
      final cachedResult = await cached();
      if (cachedResult != null) return cachedResult;
      rethrow;
    } catch (e) {
      debugPrint('ErrorHandler: Exception in $operationName: $e. Trying cache.');
      try {
        final cachedResult = await cached();
        if (cachedResult != null) return cachedResult;
      } catch (cacheErr) {
        debugPrint('ErrorHandler: Failed to read from cache for $operationName: $cacheErr');
      }
      rethrow;
    }
  }
